...
static struct partition_lookup_entry part_lookup_table[] = {
	...
	{0x58, "fsg",      "/boot/modem_fsg", "", O_RDWR|O_SYNC, -1},*/ /* Read only */

	{0x4A, "modemst1", "/boot/modem_fs1", "/dev/mmcblk0p11", O_RDWR|O_SYNC, -1},
	{0x4B, "modemst2", "/boot/modem_fs2", "/dev/mmcblk0p12", O_RDWR|O_SYNC, -1},
	{0xFF, "fsc",      "/boot/modem_fsc", "/dev/mmcblk0p14", O_RDWR|O_SYNC, -1},
	{0x58, "fsg",      "/boot/modem_fsg", "/dev/mmcblk0p18", O_RDWR|O_SYNC, -1}, /* Read only */
};
				...
				void *service_cookie)
{
	...
	//KMSGI("[%s:%d] data->path[%s]", __func__, __LINE__, data->path);
			 ...
			 struct rmt_storage_iovec_desc *xfer)
{
	...
	if (xfer->num_sector > svc.shared_mem_size_sectors) {
		KMSGI("IOVEC num sectors to write %d beyond max %d",
	...
	if ((xfer->sector_addr > svc.shared_mem_size_sectors) ||
		...
		KMSGE("IOVEC gave bad sector addr to write to %d, %d, %d",
	...
	if (svc.iovec_address_is_in_offset == 1) {
		...
		if ((offset > svc.mmap_info.shared_mem_size) ||
			...
			KMSGE("IOVEC gave bad buff offset %d, %d, %d", offset, xfer_size,
		...
		if ((start_addr < svc.mmap_info.shared_mem_physical)  ||
			...
			KMSGE("IOVEC gave bad buff addr 0x%X, 0x%X, 0x%X, 0x%X", start_addr, xfer_size,
			...
			void *service_cookie)
{
	...
	printf("%s iovec request received for %s",
...
err:
	rw_resp.resp.result = QMI_RESULT_FAILURE;
	rw_resp.resp.error = QMI_ERR_INTERNAL;
	KMSGE("Send response: res=%d err=%d", rw_resp.resp.result,
		rw_resp.resp.error);

	resp_err = qmi_csi_send_resp(req_handle, msg_id, &rw_resp, sizeof(rw_resp));
	if(resp_err != QMI_CSI_NO_ERR) {
		KMSGE("qmi_csi_send_resp returned error %d", resp_err);
		return rc;
	 } else {
		return QMI_CSI_CB_NO_ERR;
	 }
				...
				void *service_cookie)
{
	...
	KMSGI("Received req_size: %u! avail: %u", data->buff_size, svc.mmap_info.shared_mem_size);
	...
	if (data->buff_size == 0) {
		...
		KMSGI("New client making a dummy request with buffer req size 0");
	} else {
		svc.iovec_address_is_in_offset = 0;
		KMSGI("Old Client requesting buffer from RMTS Server");
	}
...
static int partition_open_helper (char *partition_path, uint32_t alternate_open_flags)
{
	...
	//else
	//KMSGI("Open %s success", partition_path);
...
static int parse_gpt_partition(void)
{
	...
	for (i = 0; i < sizeof(part_lookup_table) /
		...
		if (!part_entry->part_name[0])
			continue;
		//if(strcasecmp(part_entry->part_name,"fsg")==0)
		//	snprintf(part_path, MAX_PART_NAME, "/dev/mtdblock11");
		//else if(strcasecmp(part_entry->part_name,"modemst1")==0)
		//	snprintf(part_path, MAX_PART_NAME, "/modemst/modemst1");
		//else if(strcasecmp(part_entry->part_name,"modemst2")==0)
		//	snprintf(part_path, MAX_PART_NAME, "/modemst/modemst2");
		//else 
			//snprintf(part_path, MAX_PART_NAME, "/dev/block/platform/msm_sdcc.%d/by-name/%s",
				//MMC_BLK_SDCC_NUM, part_entry->part_name);
		//snprintf(part_path, MAX_PART_NAME, "/modemst/%s",part_entry->part_name);
		fd = partition_open_helper(part_entry->path, part_entry->alternate_open_flags);
		if (fd < 0) {
			snprintf(part_path, MAX_PART_NAME, "%s", part_entry->devpath);
			fd = partition_open_helper(part_path, part_entry->alternate_open_flags);
			if (fd < 0) {
				KMSGE("Failed to open partition %s err %d (%s). This is very bad!!!\n",
						part_path, errno, strerror(errno));
				bootdevdir = opendir("/dev/block/bootdevice/by-name");
				if(!bootdevdir) {
					KMSGE("Unable to open /dev/block/bootdevice/by-name\n");
				} else {
					KMSGE("files present within /dev/block/bootdevice/by-name:\n");
...
static int setup_client_mmap_buffer(struct uio_mmap_info *mmap_info)
{
	...
	//KMSGI("Setup mmap buffers complete\n");
	return 0;
}
...
#endif
{
	...
	KMSGI("Exiting remote storage service");
