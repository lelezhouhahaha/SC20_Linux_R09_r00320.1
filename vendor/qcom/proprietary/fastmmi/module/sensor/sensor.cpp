...
*/
static int get_input_num(const char *InputName)
{
    int num = -1;
    char input_file[50];
    char input_name[50];
    ...
    if (InputName == NULL || strlen(InputName) <= 3) {
		 ALOGE("parameter name is invalid");
		 return -1;
	}
    ...
    for (int i = 0; i < 100; i++) {
        ...
        if (fgets(input_name, sizeof(input_name) - 1, fd) != NULL) {
			fclose(fd);
			fd = NULL;
            if (strstr(input_name, InputName) != NULL) {
				ret = 1;
                num = i;
				break;
			}
...
char ql_get_sensor_data(int num, int buf[3])
{
	...
	switch (num) {
		...
		case 5:
			event_num = get_input_num("light");

			if(event_num < 0 )
			{
				printf("can not find light sensor input event\n");
				return NULL;
			}
			sprintf(input_name,"/dev/input/event%d",event_num);
