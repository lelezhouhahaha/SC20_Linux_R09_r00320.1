		...
		void *cookie)
{
	...
	switch (event) {
		case QAHW_STREAM_CBK_EVENT_WRITE_READY:
			ALOGD( "stream %d: received event - QAHW_STREAM_CBK_EVENT_WRITE_READY\n", params->stream_index);
		...
		case QAHW_STREAM_CBK_EVENT_DRAIN_READY:
			ALOGD( "stream %d: received event - QAHW_STREAM_CBK_EVENT_DRAIN_READY\n", params->stream_index);
		...
		case QAHW_STREAM_CBK_EVENT_ADSP:
			ALOGD( "stream %d: received event - QAHW_STREAM_CBK_EVENT_ADSP\n", params->stream_index);
			if (payload != NULL) {
				ALOGD( "event_type %d\n", payload[0]);
				ALOGD( "param_length %d\n", payload[1]);
				for (i=2; i* sizeof(uint32_t) <= payload[1]; i++)
					ALOGD( "param[%d] = 0x%x\n", i, payload[i]);
			}
			break;
		case QAHW_STREAM_CBK_EVENT_ERROR:
			ALOGD( "stream %d: received event - QAHW_STREAM_CBK_EVENT_ERROR\n", params->stream_index);
			stop_playback = true;
			break;
		default:
			break;
...
void *proxy_read (void* data)
{
	...
	if (rc) {
		ALOGD( "Could not open input stream %d \n",rc);
	...
	if (in_handle != NULL) {
		...
		if ((fp = fopen(params->acp.file_name,"w"))== NULL) {
			ALOGD( "Cannot open file to dump proxy data\n");
		...
		while (!(params->acp.thread_exit)) {
			...
			if (rc > 0) {
				...
				if(bytes_wrote < in_buf.bytes) {
					stop_playback = true;
					ALOGD( "Error in fwrite due to no memory(%d)=%s\n",ferror(fp), strerror(ferror(fp)));
					break;
				}
			}
		}
...
void *drift_read(void* data)
{
	...
	while (!(params->thread_exit)) {
		...
		if (params->enable_drift_correction &&
			...
			if (rc < 0)
				ALOGD( "qahw_out_set_param_data failed with err %d %d\n",
						rc, __LINE__);
		}
	}
	return NULL;
...
void *start_stream_playback (void* stream_data)
{
	...
	if (rc) {
		ALOGD("stream %d: could not open output stream, error - %d , %d\n", params->stream_index, rc, params->output_device);
		fprintf(stderr, "stream %d: could not open output stream, error - %d \n", params->stream_index, rc);
		return NULL;
	}

	ALOGD( "stream %d: open output stream is success, out_handle %p\n", params->stream_index, params->out_handle);
	...
	if (kpi_mode == true) {
		...
		if (rc) {
			ALOGD( "stream %d: could not close output stream, error - %d \n", params->stream_index, rc);
		}
		return NULL;
	}
	...
	switch(params->filetype) {
		...
		case FILE_FLAC:
		case FILE_APE:
		ALOGD( "%s:calling setparam for kvpairs\n", __func__);
		if (!(params->kvpair_values)) {
			ALOGD( "stream %d: error!!No metadata for the clip\n", params->stream_index);
		...
		if(rc){
			ALOGD( "stream %d: failed to set kvpairs\n", params->stream_index);
			return NULL;
		}
		ALOGD( "stream %d: kvpairs are set\n", params->stream_index);
		break;
		case FILE_DTS:
	...
	if (is_offload) {
		ALOGD( "stream %d: set callback for offload stream for playback usecase\n", params->stream_index);
	...
	if (params->ethread_func &&
		...
		if (rc < 0) {
			ALOGD( "stream %d: could not create effect command thread!\n", params->stream_index);
			return NULL;
		}

		ALOGD( "stream %d: loading effects\n", params->stream_index);
	...
	if (params->output_device & AUDIO_DEVICE_OUT_PROXY) {
		...
		ALOGD( "stream %d: create thread to read data from proxy\n", params->stream_index);
		...
		ALOGD( "create thread to read avtimer vs device drift\n");
		...
		if (!rc)
			...
			ALOGD( "drift query thread creation failure %d\n", rc);
	}

	rc = qahw_out_set_volume(params->out_handle, vol_level, vol_level);
	if (rc < 0) {
		ALOGD( "stream %d: unable to set volume\n", params->stream_index);
	}
	...
	if (params->pan_scale_ctrl == QAHW_PARAM_OUT_MIX_MATRIX_PARAMS) {
		...
		if (rc != 0) {
			ALOGD( "QAHW_PARAM_OUT_MIX_MATRIX_PARAMS could not be sent!\n");
		}
	}
	if (params->mix_ctrl == QAHW_PARAM_CH_MIX_MATRIX_PARAMS) {
		...
		if (rc != 0) {
			ALOGD( "QAHW_PARAM_CH_MIX_MATRIX_PARAMS could not be sent!\n");
		}
	}
	...
	if (params->set_params) {
		...
		if (rc) {
			ALOGD( "stream %s: failed to set kvpairs\n", params->set_params);
		}
	}
	// 	if (stop_playback)
	// 	{
	...
	while (!exit && !stop_playback) {
		...
		//ALOGD( "stream %d: writing to hal %zd bytes, offset %d, write length %zd\n",
		...
		latency = qahw_out_get_latency(params->out_handle);
		//ALOGD( "stream %d: bytes_written %zd, bytes_remaining %zd latency %d\n",
				//params->stream_index, bytes_written, bytes_remaining, latency);
	}


	if (params->ethread_data != nullptr) {
		ALOGD( "stream %d: un-loading effects\n", params->stream_index);
		// disable effect
	...
	if (proxy_thread_active) {
		...
		ALOGD( "wait for proxy thread exit\n");
	...
	if (rc) {
		ALOGD( "stream %d: out standby failed %d \n", params->stream_index, rc);
		fprintf(stderr, "stream %d: out standby failed %d \n", params->stream_index, rc);
	}

	ALOGD( "stream %d: closing output stream\n", params->stream_index);
	rc = qahw_close_output_stream(params->out_handle);
	if (rc) {
		ALOGD( "stream %d: could not close output stream, error - %d \n", params->stream_index, rc);
	...
	ALOGD( "stream %d: stream closed\n", params->stream_index);
	ALOGD( "stream %d: is_dual_main- %d\n", params->stream_index,is_dual_main);
	if (is_dual_main) {
		usleep(500000);
		pthread_mutex_lock(&dual_main_lock);
		ALOGD( "Dual main signal as we reached end of current running stream\n");
		is_dual_main = false;
		pthread_cond_signal(&dual_main_cond);
		pthread_mutex_unlock(&dual_main_lock);
	}
...
audio_format_t get_aac_format(int filetype, aac_format_type_t format_type)
{
	...
	if (filetype == FILE_AAC_ADTS) {
		...
		ALOGD( "Invalid filetype provided %d\n", filetype);
	...
	switch (stream_info->filetype) {
		...
		case FILE_AAC_LATM:
			if (!is_valid_aac_format_type(stream_info->aac_fmt_type)) {
				ALOGD( "Invalid format type for AAC %d\n", stream_info->aac_fmt_type);
		...
		case FILE_APE:
			...
			break;
		default:
			ALOGD( "Does not support given filetype\n");
...
int measure_kpi_values(qahw_stream_handle_t* out_handle, bool is_offload) {
	...
	if (is_offload) {
		ALOGD( "Set callback for offload stream in kpi mesaurement usecase\n");
	...
	if (fd_latency_node) {
		...
		if (ret<1)
			ALOGD( "error(%d) writing to debug node!", ret);
		fprintf(stderr, "error(%d) writing to debug node!", ret);
		fflush(fd_latency_node);
	} else {
		ALOGD( "debug node(%s) open failed!", LATENCY_NODE);
	...
	while (count < 64) {
		...
		if (count == 0) {
			...
			if (ret) {
				ALOGD( "error(%d) fetching start time for cold latency", ret);
			...
			if (ret) {
				ALOGD( "error(%d) fetching start time for continuous latency", ret);
		...
		bytes_remaining -= bytes_written;
		ALOGD( "bytes_written %zd, bytes_remaining %zd\n",
				bytes_written, bytes_remaining);
	...
	ALOGD( " cold latency %llums, continuous latency %llums,\n", tcold, tcont);
...
int tigger_event(qahw_stream_handle_t* out_handle)
{
	...
	ALOGD( "Set callback for event trigger usecase\n");
	ret = qahw_out_set_callback(out_handle, async_callback,
			&stream_param[PRIMARY_STREAM_INDEX]);
	if (ret < 0) {
		ALOGD( "qahw_out_set_callback failed with err %d\n",
				ret);
		goto done;
	}
	ALOGD( "Register for event using qahw_out_set_param_data\n");
	ret = qahw_out_set_param_data(out_handle, QAHW_PARAM_ADSP_STREAM_CMD,
			(qahw_param_payload *)&payload);
	if (ret < 0) {
		ALOGD( "qahw_out_set_param_data failed with err %d\n",
				ret);
		goto done;
	}
	ALOGD( "qahw_out_set_paramdata succeeded\n");
...
static int detect_stream_params(stream_config *stream) {
	...
	if (rc) {
		ALOGD( "stream could not be opened\n");
	...
	if ((stream->config.sample_rate = get_rate(param_string)) <= 0) {
		ALOGD( "Unable to extract sample rate val =(%d) string(%s)\n", stream->config.sample_rate, param_string);
	...
	if ((ch = get_channels(param_string)) <= 0) {
		ALOGD( "Unable to extract channels =(%d) string(%s)\n", ch, param_string == NULL ? "null":param_string);
	...
	if ((stream->config.format = get_pcm_format(param_string)) <= 0) {
		ALOGD( "Unable to extract pcm format val =(%d) string(%s)\n", stream->config.format, param_string);
	 ...
	 */
	ALOGD( "%s:closing the usb stream\n", __func__);
	...
	if (rc) {
		ALOGD( "%s:stream could not be closed\n", __func__);
...
int get_wav_header_length (FILE* file_stream)
{
	...
	if(fread(&subchunk_size, 4, 1, file_stream) != 1) {
		ALOGD( "Unable to read subchunk:\n");
		fprintf(stderr, "Unable to read subchunk:\n");
		exit (1);
	}
	if(subchunk_size < 16) {
		ALOGD( "This is not a valid wav file \n");
	...
	if (file_type == FILE_DTS) {
		...
		while (!feof(file) &&
			...
			if (!ret) {
				...
				ALOGD( "Identified chunk of %c %c %c %c %c %c %c %c \n",
	...
	if ((AUDIO_DEVICE_IN_USB_DEVICE == dev) ||
		...
		if (!usb_hal_handle) {
			...
			if ((usb_hal_handle = qahw_load_module(QAHW_MODULE_ID_USB)) == NULL) {
				ALOGD("failure in Loading usb HAL\n");
...
int unload_hals() {
	...
	if (usb_hal_handle) {
		...
		if (qahw_unload_module(usb_hal_handle) < 0) {
			ALOGD("failure in Un Loading usb HAL\n");
			fprintf(stderr,"failure in Un Loading usb HAL\n");
			return -1;
		}
	}
	if (bt_hal_handle) {
		ALOGD("UnLoading BT HAL\n");
		if (qahw_unload_module(bt_hal_handle) < 0) {
			ALOGD("failure in UnLoading BT HAL\n");
	...
	if (primary_hal_handle) {
		...
		if (qahw_unload_module(primary_hal_handle) < 0) {
			ALOGD("failure in Un Loading Primary HAL\n");
	...
	for (i = 0; i < num_of_streams; i++) {
		...
		if (stream->filename) {
			if ((stream->file_stream = fopen(stream->filename, "r"))== NULL) {
				ALOGD( "Cannot open audio file %s\n", stream->filename);
...
int audio_playback(int argc, char* argv[]) {
	...
	while ((opt = getopt_long(argc,
		...
		switch (opt) {
			...
			case 'r':
				...
				{
					stream_param[i].config.offload_info.sample_rate = 44100;
					stream_param[i].config.sample_rate = 44100;
				}
				else
				{
			...
			case 'c':
				...
				{
					stream_param[i].channels = 2;
					stream_param[i].config.channel_mask = 2;
				}
				else
				{
			...
			case 'k':
				...
				if(strncmp(mixer_ctrl_name, "downmix", 7) == 0) {
					...
					for (iter_i= 0; iter_i < stream_param[i].mm_params_downmix.num_input_channels; iter_i++) {
						ALOGD( "0x%x, ", stream_param[i].mm_params_downmix.input_channel_map[iter_i]);
					}
					...
					for (iter_i = 0; iter_i < stream_param[i].mm_params_downmix.num_output_channels; iter_i++)
						ALOGD( "0x%x, ", stream_param[i].mm_params_downmix.output_channel_map[iter_i]);
					...
					for (iter_i = 0; iter_i < stream_param[i].mm_params_downmix.num_output_channels; iter_i++){
						for (iter_j = 0; iter_j < stream_param[i].mm_params_downmix.num_input_channels; iter_j++){
							ALOGD( "%.2f ",stream_param[i].mm_params_downmix.mixer_coeffs[iter_i][iter_j]);
						}
						ALOGD( "\n");
					}
					...
					for (iter_i= 0; iter_i < stream_param[i].mm_params_pan_scale.num_input_channels; iter_i++) {
						ALOGD( "0x%x, ", stream_param[i].mm_params_pan_scale.input_channel_map[iter_i]);
					}
					...
					for (iter_i = 0; iter_i < stream_param[i].mm_params_pan_scale.num_output_channels; iter_i++)
						ALOGD( "0x%x, ", stream_param[i].mm_params_pan_scale.output_channel_map[iter_i]);
					...
					for (iter_i = 0; iter_i < stream_param[i].mm_params_pan_scale.num_output_channels; iter_i++){
						for (iter_j = 0; iter_j < stream_param[i].mm_params_pan_scale.num_input_channels; iter_j++){
							ALOGD( "%.2f ",stream_param[i].mm_params_pan_scale.mixer_coeffs[iter_i][iter_j]);
						}
						ALOGD( "\n");
					}
			...
			case 'e':
				...
				if (stream_param[i].effect_index < 0 || stream_param[i].effect_index >= EFFECT_MAX) {
					...
					ALOGD( "visualizer effect testing is not available\n");
					stream_param[i].effect_index = -1;
				} else {
			...
			case 'P':
				if(i >= MAX_PLAYBACK_STREAMS - 1) {
					ALOGD( "cannot have more than %d streams\n", MAX_PLAYBACK_STREAMS);
					fprintf(stderr, "cannot have more than %d streams\n", MAX_PLAYBACK_STREAMS);
					return 0;
				}
				i++;
				ALOGD( "Stream index incremented to %d\n", i);
				break;
			case 'u':
			...
			case 'C':
				...
				if (optind < argc && *argv[optind] != '-') {
					...
					ALOGD( " Device config ::::  sample_rate - %d \n", device_cfg_params.sample_rate);
				...
				if (optind < argc && *argv[optind] != '-') {
					...
					optind++;
					ALOGD( " Device config :::: channels - %d \n", device_cfg_params.channels);
				...
				if (optind < argc && *argv[optind] != '-') {
					...
					optind++;
					ALOGD( " Device config :::: bit_width - %d \n", device_cfg_params.bit_width);
				...
				if (optind < argc && *argv[optind] != '-') {
					...
					optind++;
					ALOGD( " Device config :::: channel_allocation - %d \n", device_cfg_params.channel_allocation);
	 ...
	 */
	ALOGD( "ADL: Starting audio hal tests for streams: %d\n", num_of_streams);

	if (kpi_mode == true && num_of_streams > 1) {
		ALOGD( "kpi-mode is not supported for multi-playback usecase\n");
		goto exit;
	} else if (event_trigger == true && num_of_streams > 1) {
		ALOGD( "event_trigger is not supported for multi-playback usecase\n");
		goto exit;
	}

	if (num_of_streams > 1 && stream_param[num_of_streams-1].output_device & AUDIO_DEVICE_OUT_PROXY) {
		ALOGD( "Proxy thread is not supported for multi-playback usecase\n");
		goto exit;
	}
	...
	if (num_of_streams >= 2) {
		...
		for(i = 0; i < num_of_streams; i++) {
			ALOGD( "is_dual_main - %d  stream_param[i].flags - %d\n", is_dual_main, stream_param[i].flags);
	...
	for (i = 0; i < num_of_streams; i++) {
		...
		if ((kpi_mode == false) &&
			...
			if (stream_param[PRIMARY_STREAM_INDEX].filename == nullptr) {
				ALOGD( "Primary file name is must for non kpi-mode\n");
				goto exit;
			}
		}
		...
		if (stream->filename) {
			...
			ALOGD( "Playing from:%s\n", stream->filename);
			get_file_format(&stream_param[i]);
		} else if (AUDIO_DEVICE_NONE != stream->input_device) {
			ALOGD( "Playing from device:%x\n", stream->input_device);
			ALOGD( "Playing from url:%s\n", stream->device_url);
			ALOGD( "setting up input hal and stream:%s\n", stream->device_url);
			...
			if (rc) {
				ALOGD( "input stream could not be re-opened\n");
				return rc;
			}
		...
		if (stream->output_device & AUDIO_DEVICE_OUT_PROXY)
			ALOGD( "Saving pcm data to file: %s\n", proxy_params.acp.file_name);
		...
		if (send_device_config) {
			...
			if (rc != 0) {
				ALOGD( "Set Device Config Failed\n");
		...
		if (rc) {
			ALOGD( "stream %d: failed to create thread\n", stream->stream_index);
			fprintf(stderr, "stream %d: failed to create thread\n", stream->stream_index);
			exit(0);
		}
		thread_active[i] = true;
	...
	for (i = 0; i < num_of_streams; i++) {
		...
		if (stream_param[i].file_stream != nullptr)
			...
			if (stream != NULL && stream->in_handle) {
				rc = qahw_close_input_stream(stream->in_handle);
				if (rc) {
					ALOGD( "input stream could not be closed\n");
...
void QL_Audio_Playback(struct Audio_Playback playback, char *file)
{
	...
	sprintf(channel, "%d", playback.channel);
	sprintf(filetype, "%d", playback.filetype);
	sprintf(device, "%d", playback.device);
	sprintf(volume, "%d", playback.volume);
	sprintf(rate, "%d", playback.rate);
	if (playback.filetype == Wav) {
		ptr = strstr(file, ".wav");
		if (ptr == NULL)
			snprintf(filebuf, sizeof(filebuf), "%s%s", file, ".wav");
		else
			snprintf(filebuf, sizeof(filebuf), "%s", file);
	} else if (playback.filetype == Mp3) {
		ptr = strstr(file, ".mp3");
		if (ptr == NULL)
			snprintf(filebuf, sizeof(filebuf), "%s%s", file, ".mp3");
		else
			snprintf(filebuf, sizeof(filebuf), "%s", file);
	} else {
		ALOGE("Bad file type:%d", playback.filetype);
		return;
	}
	ALOGE("audio wav file:%s", filebuf);

	char  *str[30]={NULL,"-c",channel,"-t",filetype,"-f", filebuf,"-d",device,"-v",volume,"-r",rate, "-F", "2"};

	audio_playback(15,str);
}
