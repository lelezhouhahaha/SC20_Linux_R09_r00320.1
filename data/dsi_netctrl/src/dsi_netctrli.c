...
#else
  ret = dsi_config_load_ex_ap(target_str);
#endif /* FEATURE_EXTERNAL_AP */
...
static int wds_adjust_profile(dsi_store_t *hndl, qmi_client_type wds_clnt,
        wds_profile_type_enum_v01 profile_type)
{
	...
	wds_delete_profile_req_msg_v01 delete_profile_req;
	wds_delete_profile_resp_msg_v01 delete_profile_resp;
    int i;
	...
	if (hndl->priv.start_nw_params.profile_index_valid)
		profile_idx = hndl->priv.start_nw_params.profile_index;
    ...
    if(NULL != plist_resp)
    {
        ...
        for(i=0; i<plist_resp->profile_list_len; i++)
        {
            if(plist_resp->profile_list[i].profile_index == profile_idx)
            {
                ...
                DSI_LOG_ERROR("Delete profile_index:%d", profile_idx);

				delete_profile_req.profile.profile_type = profile_type;
				delete_profile_req.profile.profile_index = profile_idx;
				ret = qmi_client_send_msg_sync(wds_clnt, QMI_WDS_DELETE_PROFILE_REQ_V01,
						&delete_profile_req, sizeof(delete_profile_req),
						&delete_profile_resp, sizeof(delete_profile_resp), 3000);
				if(QMI_NO_ERR != ret || QMI_RESULT_SUCCESS_V01 != delete_profile_resp.resp.result) {
					DSI_LOG_ERROR("Delete profile index %d failure: error %d",
							profile_idx, delete_profile_resp.resp.error);
					free(plist_resp);
					return -3;
				}
    ...
    for(i=0; i < profile_idx; i++)
    {
		...
		if (hndl->priv.start_nw_params.apn_name_valid)
			strncpy(req.apn_name, hndl->priv.start_nw_params.apn_name,
					strlen(hndl->priv.start_nw_params.apn_name));
		else
			strncpy(req.apn_name, "3gpp", 4);//3gpp

		DSI_LOG_DEBUG( "%s", "Peeta b3" );
...
int dsi_profile_adjust(dsi_store_t *hndl)
{
	...
	DSI_LOG_DEBUG( "%s", "profile_index_valid=%d, profile_index=%d",
			hndl->priv.start_nw_params.profile_index_valid,
    ...
    for (modem=0; modem<DSI_MAX_MODEMS; modem++)
    {
        ...
        if(0 == wds_adjust_profile(hndl, DSI_MODEM_GET_DEFAULT_WDS_HNDL(modem),
...
static int wds_get_apn_name_by_profile_index(qmi_client_type wds_clnt,
        unsigned char profile_idx, char *apn_name)
{
	int ret = 0;
	...
	//DSI_LOG_DEBUG( "%s", "Peeta e3" );
	if (profile_idx <= 0)
		return -1;

	memset(&req, 0x0, sizeof(wds_get_profile_settings_req_msg_v01));
	memset(&resp, 0x0, sizeof(wds_get_profile_settings_resp_msg_v01));

	req.profile.profile_type = 0; //DATA_CALL_PROFILE_TYPE_3GPP_V01
	req.profile.profile_index = profile_idx;

	ret = qmi_client_send_msg_sync(wds_clnt, QMI_WDS_GET_PROFILE_SETTINGS_REQ_V01,
			&req, sizeof(req), &resp, sizeof(resp), 2000);

	if(QMI_NO_ERR != ret || QMI_RESULT_SUCCESS_V01 != resp.resp.result) {
        DSI_LOG_ERROR("Failed to get apn name %d, ret=%d, result=%d, err=%d",
			profile_idx, ret, resp.resp.result, resp.resp.error);
		return -2;
	}

	if (ret == DSI_SUCCESS) {
		if (req.profile.profile_type == 0) {
			if (resp.apn_name_valid) {
				snprintf(apn_name, 150, "%s", resp.apn_name);
				return 0;
			}
		}
	}
...
int dsi_profile_get_apn_name(dsi_store_t *hndl, unsigned char profile_idx, char *apn_name)
{
	int ret = -1;
	int modem = 0;

	//DSI_LOG_DEBUG( "%s", "Peeta e4" );
	for (modem = 0; modem < DSI_MAX_MODEMS; modem++) {
		if (DSI_MODEM_GET_DEFAULT_WDS_HNDL(modem) == DSI_INVALID_WDS_HNDL) {
			continue;
		}

		if (wds_get_apn_name_by_profile_index(DSI_MODEM_GET_DEFAULT_WDS_HNDL(modem),
					profile_idx, apn_name) == 0) {
			return 0;
		}
	}
