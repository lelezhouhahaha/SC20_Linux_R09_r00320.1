...
)
{
    ...
    switch (msg_id)
    {
        ...
        case MCM_NW_EVENT_REGISTER_REQ_V01:
            ...
            //UTIL_LOG_MSG("Peeta[%d]: register_signal_strength_event_valid = %d, register_signal_strength_event = %d\n",
             //       __LINE__, nw_req_msg->register_signal_strength_event_valid,
              //      nw_req_msg->register_signal_strength_event);
...
)
{
    ...
    UTIL_LOG_MSG("number_of_events:%d, number_of_message_id=%d\n",
            event_list->num_of_events, number_of_message_id);
    number_of_message_id = (number_of_message_id < event_list->num_of_events) ?
        event_list->num_of_events:number_of_message_id;
#endif
    ...
    for (iter_evt_list = 0; iter_evt_list < (event_list->num_of_events); iter_evt_list ++)
    {
        ...
        for (iter_msg_id = 0; iter_msg_id < MCM_MAX_NUM_MSG_ID; iter_msg_id ++)
        {
        ...
        if (1 != msg_id_registered && event_id != 0) {
            for (iter_msg_id = 0; iter_msg_id < MCM_MAX_NUM_MSG_ID; iter_msg_id++) {
                if (mcm_srv_client_hdl[iter_srv].client_handles[iter_client].message_id[iter_msg_id] == 0) {
                    mcm_srv_client_hdl[iter_srv].client_handles[iter_client].message_id[iter_msg_id] = event_id;
                    mcm_srv_client_hdl[iter_srv].client_handles[iter_client].num_of_message_id++;
                    UTIL_LOG_MSG("Peeta[%d] Register event_id:%d, iter_msg_id = %d\n", __LINE__, event_id, iter_msg_id);
                    break;
                }
            }
        }
    ...
    //UTIL_LOG_MSG("total number of message_id:%d, num_of_message_id=%d\n",
     //       number_of_message_id, mcm_srv_client_hdl[iter_srv].client_handles[iter_client].num_of_message_id);
...
)
{
    ...
    //number_of_message_id = mcm_srv_client_hdl[iter_srv].client_handles[iter_client].num_of_message_id;
    //UTIL_LOG_MSG("number_of_message_id:%d, num_of_events=%d\n", number_of_message_id ,event_list->num_of_events);
    for (iter_evt_list = 0; iter_evt_list < (event_list->num_of_events); iter_evt_list ++)
    {
        event_id = *(event_list->event_id);
        /* check if event_id already registered */
        //UTIL_LOG_MSG("Peeta[%d] event_id:%d\n", __LINE__, event_id);
        for (iter_msg_id = 0; iter_msg_id < MCM_MAX_NUM_MSG_ID; iter_msg_id ++)
        {
            //UTIL_LOG_MSG("Peeta[%d] message_id:%d\n", __LINE__,
             //       mcm_srv_client_hdl[iter_srv].client_handles[iter_client].message_id[iter_msg_id]);
           if (mcm_srv_client_hdl[iter_srv].client_handles[iter_client].message_id[iter_msg_id] == event_id) {
               /* event_id exists */
               //msg_id_registered = 1;
                UTIL_LOG_MSG("Peeta[%d] Unregister event_id:%d\n", __LINE__,
                        mcm_srv_client_hdl[iter_srv].client_handles[iter_client].message_id[iter_msg_id]);
                mcm_srv_client_hdl[iter_srv].client_handles[iter_client].message_id[iter_msg_id] = 0;
                mcm_srv_client_hdl[iter_srv].client_handles[iter_client].num_of_message_id --;
                break;
           }
        }
        
        if (mcm_srv_client_hdl[iter_srv].client_handles[iter_client].num_of_message_id <= 0)
            break;
        event_list->event_id++;
    }
...
)
{
    ...
    if (iter_srv == MCM_MAX_SERVICES)
    {//没有直接返回说明匹配到了对应的service_id
        UTIL_LOG_MSG("mcm_srv_send_indication_to_all_registered_clients EXIT %d\n", __LINE__);
        return;
    }
    ...
    for(iter_client=0; iter_client<MCM_MAX_CLIENTS; iter_client++)
    {
        ...
        //UTIL_LOG_MSG("peeta[%d]: iter_client = %d, num_of_msg_id=%d\n", __LINE__, iter_client, num_of_msg_id);
        for (iter_msg_id_list = 0; iter_msg_id_list < MCM_MAX_NUM_MSG_ID; iter_msg_id_list ++)
        {
            //UTIL_LOG_MSG("peeta[%d]: iter_client=%d, message_id[iter_msg_id_list]=%d\n",
             //       __LINE__, iter_client,
              //      mcm_srv_client_hdl[iter_srv].client_handles[iter_client].message_id[iter_msg_id_list]);
            if (message_id == mcm_srv_client_hdl[iter_srv].client_handles[iter_client].message_id[iter_msg_id_list])
            {
                ...
                mcm_srv_post_indication(mcm_srv_client_hdl[iter_srv].client_handles[iter_client].client_handle,
                                        ...
                                        ind_c_struct,
                                        ind_c_struct_len);
            }//没有进来说明没有在对应的client中找到对应的message_id
        }
    }
    UTIL_LOG_MSG("mcm_srv_send_indication_to_all_registered_clients EXIT %d\n", __LINE__);
}
