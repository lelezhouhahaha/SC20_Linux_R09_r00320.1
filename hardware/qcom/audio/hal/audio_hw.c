                      ...
                      snd_device_t snd_device)
{
    ...
    if (platform_can_enable_spkr_prot_on_device(snd_device) &&
       ...
       if (audio_extn_spkr_prot_start_processing(snd_device)) {
        ...
        ALOGD("[%s:%d] snd_device(%d: %s)", __func__, __LINE__, snd_device, device_name);
                                              ...
                                              snd_device_t snd_device)
{
	...
	ALOGD("%s:Enter snd_device[%d]", __func__, snd_device);
                                            ...
                                            audio_usecase_t uc_id)
{
    ...
    list_for_each_safe(node, tmp_node, &adev->usecase_list) {
        usecase = node_to_item(node, struct audio_usecase, list);
        if (usecase->id == uc_id)
            return usecase;
    }
...
static audio_usecase_t get_offload_usecase(struct audio_device *adev, bool is_compress)
{
    ...
    ALOGD("%s: num_usecase: %d", __func__, num_usecase);
    ...
    ALOGD("%s: offload usecase is %d", __func__, ret_uc);
    return ret_uc;
}
...
static int out_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
    ...
    ALOGD("%s: enter: usecase(%d: %s) kvpairs: %s", __func__, out->usecase, use_case_table[out->usecase], kvpairs);
        ...
        while (supported_channel_masks[i] != 0) {
            for (j = 0; j < ARRAY_SIZE(channels_name_to_enum_table); j++) {
                if (channels_name_to_enum_table[j].value == supported_channel_masks[i]) {
                    ...
                    strncat(value, channels_name_to_enum_table[j].name, sizeof(value));
                    first = false;
                    break;
                }
            }
        ...
        while (supported_formats[i] != 0) {
            for (j = 0; j < ARRAY_SIZE(formats_name_to_enum_table); j++) {
                if (formats_name_to_enum_table[j].value == supported_formats[i]) {
                    ...
                    strncat(value, formats_name_to_enum_table[j].name, sizeof(value));
                    first = false;
                    break;
                }
            }
...
static char* out_get_parameters(const struct audio_stream *stream, const char *keys)
{
    ...
    if (ret >= 0) {
        ...
        if (out->flags & AUDIO_OUTPUT_FLAG_DIRECT &&
            ...
            ALOGV("in direct_pcm");
            strncat(value, "true", sizeof(value));
        } else {
            ALOGV("not in direct_pcm");
            strncat(value, "false", sizeof(value));
        }
        ...
        while (out->supported_sample_rates[i] != 0) {
            for (j = 0; j < ARRAY_SIZE(out_sample_rates_name_to_enum_table); j++) {
                if (out_sample_rates_name_to_enum_table[j].value == out->supported_sample_rates[i]) {
                    ...
                    strncat(value, out_sample_rates_name_to_enum_table[j].name, sizeof(value));
                    first = false;
                    break;
                }
            }
                              ...
                              float right)
{
    ...
    if (left != right) {
		...
		volume = (int) (left * 100);
		ALOGD("[%s:%d] left = %f volume = %d", __func__, __LINE__, left, volume);
        pcm_device_id = platform_get_pcm_device_id(out->usecase, PCM_PLAYBACK);
        //ALOGD("[%s:%d] device [0x%X]", __func__, __LINE__, out->devices);
		snprintf(mixer_ctl_name, sizeof(mixer_ctl_name), "Playback %d Volume", pcm_device_id);

		ALOGD("[%s:%d] mixer_ctl_name %s", __func__, __LINE__, mixer_ctl_name);
		ctl = mixer_get_ctl_by_name(adev->mixer, mixer_ctl_name);
		if (!ctl) {
		...
		ret = mixer_ctl_set_value(ctl, 0, (int)(left * 10 * PCM_PLAYBACK_VOLUME_MAX)/2);
		if (ret < 0) {
			ALOGW("%s: Could not set ctl, error:%d ", __func__, ret);
			//return -EINVAL;
		}

		if (out->devices & AUDIO_DEVICE_OUT_SPEAKER) {
			snprintf(mixer_ctl_name, sizeof(mixer_ctl_name), "RX3 Digital Volume", pcm_device_id);
			//ALOGD("[%s:%d] mixer_ctl_name %s", __func__, __LINE__, mixer_ctl_name);
			ctl = mixer_get_ctl_by_name(adev->mixer, mixer_ctl_name);
			if (!ctl) {
				ALOGE("%s : Could not get ctl for mixer cmd - %s", __func__, mixer_ctl_name);
			...
			ret = mixer_ctl_set_value(ctl, 0, volume);
			if (ret < 0) {
				ALOGE("%s: Could not set ctl, error:%d ", __func__, ret);
			}
		}

		if ((out->devices & AUDIO_DEVICE_OUT_WIRED_HEADPHONE) ||
				(out->devices & AUDIO_DEVICE_OUT_WIRED_HEADSET)) {
			snprintf(mixer_ctl_name, sizeof(mixer_ctl_name), "RX2 Digital Volume", pcm_device_id);
			//ALOGD("[%s:%d] mixer_ctl_name %s", __func__, __LINE__, mixer_ctl_name);
			ctl = mixer_get_ctl_by_name(adev->mixer, mixer_ctl_name);
			if (!ctl) {
				ALOGE("%s : Could not get ctl for mixer cmd - %s", __func__, mixer_ctl_name);
		...
		if (out->devices & AUDIO_DEVICE_OUT_EARPIECE) {
			snprintf(mixer_ctl_name, sizeof(mixer_ctl_name), "RX1 Digital Volume", pcm_device_id);
			//ALOGD("[%s:%d] mixer_ctl_name %s", __func__, __LINE__, mixer_ctl_name);
			ctl = mixer_get_ctl_by_name(adev->mixer, mixer_ctl_name);
			if (!ctl) {
				ALOGE("%s : Could not get ctl for mixer cmd - %s", __func__, mixer_ctl_name);
        ...
        ALOGD("%s:Pcm set volume value:%d, volume:%f", __func__, volume, left);
                            ...
                            const char *address __unused)
{
    ...
    *stream_out = NULL;
    ...
    ALOGD("%s: format(%#x) sample_rate(%d) channel_mask(%#x) devices(%#x) flags(%#x) stream_handle(%p)",
			__func__, config->format, config->sample_rate, config->channel_mask, devices, flags, &out->stream);
    ...
    if ((out->dev->mode == AUDIO_MODE_IN_COMMUNICATION || voice_extn_compress_voip_is_active(out->dev)) &&
        ...
        if (out->flags & AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD) {
            ...
            ALOGD("Compress Offload usecase .. usecase selected %d", out->usecase);
        } else {
            out->usecase = get_offload_usecase(adev, false /* is_compress */);
            ALOGD("non-offload DIRECT_usecase ... usecase selected %d ", out->usecase);
        }
        ...
        create_offload_callback_thread(out);
          ...
          ALOGD("[%s:%d] flags & AUDIO_OUTPUT_FLAG_INCALL_MUSIC", __func__, __LINE__);
		...
		ALOGD("[%s:%d] output devices = %d", __func__, __LINE__, out->devices);
    ...
    ALOGD("%s devices:%d, format:%x, out->sample_rate:%d,out->bit_width:%d out->format:%d out->flags:%x, flags: %x usecase %d",
                                  ...
                                  audio_source_t source)
{
    ...
    *stream_in = NULL;
    ...
    ALOGI("sample_rate(%d) channel_mask(%#x) devices(%#x)\
        stream_handle(%p) io_handle(%d) source(%d) format %x", config->sample_rate,
...
int adev_create_audio_loopback(struct audio_hw_device *dev,
                            uint32_t input,
                            ...
                            const struct audio_config *sink)
{
	int ret;
	ALOGD("[%s:%d] Enter\n", __func__, __LINE__);
     ret = audio_extn_hw_loopback_create_audio_loopback(dev,
                                         input,
                                         source,
                                         output,
                                         sink);
...
int adev_release_audio_loopback(struct audio_hw_device *dev)
{
    return audio_extn_hw_loopback_release_audio_loopback(dev);
}
                     ...
                     hw_device_t **device)
{
    ...
    adev->device.create_audio_loopback = adev_create_audio_loopback;
    adev->device.release_audio_loopback = adev_release_audio_loopback;
