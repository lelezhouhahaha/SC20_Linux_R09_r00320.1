...
int32_t qti_audio_server::onTransact(uint32_t code,
    ...
    switch(code) {
        ...
        case CREATE_AUD_LOOPBACK: {
            qahw_module_handle_t *hw_module = NULL;
            data.read(&hw_module, sizeof(qahw_module_handle_t *));
            uint32_t input = data.readUint32();
            android::Parcel::ReadableBlob blob;
            uint32_t blob_size;
            data.readUint32(&blob_size);
            data.readBlob(blob_size, &blob);
            audio_config_t source;
            memset((void *)&source, 0x0, sizeof(audio_config_t));
            memscpy((void *)&source, blob_size, blob.data(), blob_size);

            uint32_t output = data.readUint32();
            data.readUint32(&blob_size);
            data.readBlob(blob_size, &blob);
            audio_config_t sink;
            memset((void *) &sink, 0x0, sizeof(audio_config_t));
            memscpy((void *) &sink, blob_size, blob.data(), blob_size);
            blob.release();

            return qahw_create_audio_loopback(hw_module, input,
                                         &source, output, &sink);
        }
        break;

        case RELEASE_AUD_LOOPBACK: {
            qahw_module_handle_t *hw_module = NULL;
            data.read(&hw_module, sizeof(qahw_module_handle_t *));
            return qahw_release_audio_loopback(hw_module);
        }
        break;
        case GET_VERSION: {
...
int qti_audio_server::qahw_create_audio_loopback(qahw_module_handle_t *hw_module,
                        uint32_t input,
...
{
	int ret;
    ret = qahw_create_audio_loopback_l(hw_module, input, source, output, sink);
	return ret;
}

int qti_audio_server::qahw_release_audio_loopback(qahw_module_handle_t *hw_module)
{
    return qahw_release_audio_loopback_l(hw_module);
}
