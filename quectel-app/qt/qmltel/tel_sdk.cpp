...
ql_mcm_voice_call_ind *pVoiceCallInd;
bool flag = false;static bool flag_call = false;static bool flag_stop = false;
void ql_voice_call_ind_func(unsigned int ind_id,
                                   void* ind_data,
                                   uint32_t ind_data_len)
{
    ...
    switch(ind_id)
    {
        case E_QL_MCM_VOICE_CALL_IND:
        {
            ...
            pVoiceCallInd = (ql_mcm_voice_call_ind*)ind_data;
            ...
            for(i = 0; i < pVoiceCallInd->calls_len; i++)
            {
                ...
                if(pVoiceCallInd->calls[i].state == 5)
                {
                    flag_stop = true;
                }
				if(pVoiceCallInd->calls[i].state == 0)
				{
					flag_call = true;
				}
            }
...
{
	//voice初始化
	int ret = QL_Voice_Call_Client_Init(&h_voice);
    ...
    while(1){
        ...
        //printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!thread1 is running calls_len %d\n",pVoiceCallInd->calls_len);
        pthread_mutex_lock(&mutex);
        //int ret = QL_Voice_Call_Answer(h_voice, voice_call_id);
        //printf("QL_Voice_Call_Answer ret = %d\n", ret);
		if(!flag_call)
		{
			QVariant returnedValue;
			QVariant msg = "message sended from C++wwwwwwwwwwwwwwwwwwwww";
			QMetaObject::invokeMethod(fparent, "test1", Q_RETURN_ARG(QVariant, returnedValue),Q_ARG(QVariant, msg));
		}
...
void tel_sdk::Call_State()
{
	int ret = QL_Voice_Call_AddCommonStateHandler(h_voice, (QL_VoiceCall_CommonStateHandlerFunc_t)ql_voice_call_ind_func);
	printf("QL_Voice_Call_AddCommonStateHandler ret = %d\n", ret);
	
}


void tel_sdk::cppSlotA(const QString &str)
{
    qDebug()<<"cpp slot a";
	char* ptr;
    QByteArray ba;
	...
	//scanf("%s", PhoneNum);
	int ret = QL_Voice_Call_Start(h_voice, E_QL_VCALL_EXTERNAL_SLOT_1, ptr, &voice_call_id);
	printf("QL_Voice_Call_Start ret = %d, with voice_call_id=%d\n", ret, voice_call_id);
}

void tel_sdk::cppSlotB(const QString &str, int value)
{
...
char tel_sdk::display()
{
	if(flag_call)
	{
		for(int i = 0; i < pVoiceCallInd->calls_len; i++)
        {
			printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!######### Call id=%d, PhoneNum:%s ######\n", 
                            pVoiceCallInd->calls[i].call_id, 
                            pVoiceCallInd->calls[i].number);
		}
		flag_call = false;
		...
		QMetaObject::invokeMethod(fparent, "test1", Q_RETURN_ARG(QVariant, returnedValue),Q_ARG(QVariant, pVoiceCallInd->calls[0].number));
		//return pVoiceCallInd->calls[0].number;
	}
    if(flag_stop)
    {
        flag_stop = false;
        ...
        QVariant returnedValue;
        QVariant msg = "message sended from C++";
        QMetaObject::invokeMethod(fparent, "stop", Q_RETURN_ARG(QVariant, returnedValue),Q_ARG(QVariant, pVoiceCallInd->calls[0].number));
    }
}

void tel_sdk::dial_stop()
{
	if(flag)
	{
		for(int i = 0; i < pVoiceCallInd->calls_len; i++)
		{
			int ret = QL_Voice_Call_End(h_voice,pVoiceCallInd->calls[i].call_id);
			printf("@@@@@@@@@@@@@@@@@@QL_Voice_Call_End ret = %d, with voice_call_id=%d\n", ret, pVoiceCallInd->calls[i].call_id);
		}
	}
	else{
		int ret = QL_Voice_Call_End(h_voice,voice_call_id);
		printf("QL_Voice_Call_End ret = %d, with voice_call_id=%d\n", ret, voice_call_id);
	}
...
void tel_sdk::tel_answer()
{
	if(flag)
	{
		for(int i = 0; i < pVoiceCallInd->calls_len; i++)
		{
			int ret = QL_Voice_Call_Answer(h_voice, pVoiceCallInd->calls[i].call_id);
			printf("@@@@@@@@@@@@@@@@@@QL_Voice_Call_Answer ret = %d  with voice_call_id=%d\n", ret, pVoiceCallInd->calls[i].call_id);
		}
	}
}
