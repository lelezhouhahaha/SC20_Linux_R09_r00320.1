...
st_api_test_case at_nw_testlist[] = 
{
    ...
    {16,    "QL_MCM_NW_ScreenStateHandler"},
    {17,    "QL_MCM_NW_SetSystemSelectionPreference"},
    {18,    "QL_MCM_NW_SwitchDataSubscription"},
...
static int test_nw(void)
{
    ...
    while(1)
    {
        ...
        {
            int screen_state = 0;
            printf("please input screen state(0: off, other: on): \n");
            scanf("%d", &screen_state);

            ret = QL_MCM_NW_ScreenStateHandler(h_nw, screen_state);
            printf("QL_MCM_NW_NetworkScreenStateHandler ret = %d\n", ret);
            break;
        }
        case 17:
        {
            ret = QL_MCM_NW_SetSystemSelectionPreference(h_nw);
            printf("QL_MCM_NW_SetSystemSelectionPreference ret = %d\n", ret);
            break;
        }
        case 18:
        {
            int card = 0;
            printf("please input card(0: primary, 1: secondary): \n");
            scanf("%d", &card);

            ret = QL_MCM_NW_SwitchDataSubscription(h_nw, card);
            printf("QL_MCM_NW_SwitchDataSubscription ret = %d\n", ret);
            break;
        }
