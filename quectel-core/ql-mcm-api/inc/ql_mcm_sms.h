...
typedef struct
{
    char push_content_ptr[255];
    uint16_t push_content_len;
} ql_wap_push_sms_info_t;

typedef ql_wap_push_sms_info_t       QL_WAP_PUSH_SMS_Msg_t; 
typedef ql_wap_push_sms_info_t*      QL_WAP_PUSH_SMS_MsgRef; 

/* Callback function registered to QL_SMS_AddRxMsgHandler, msgRef contains the detail msg infor */
typedef void (*QL_SMS_RxPushMsgHandlerFunc_t)(
    QL_WAP_PUSH_SMS_MsgRef      pushmsgRef
);
//add by joe end
int QL_wap_push_SMS_AddRxMsgHandler(QL_SMS_RxPushMsgHandlerFunc_t handlerPtr);
