...
int QL_LOC_Set_Indications(uint32_t h_loc, int bit_mask);

/* Set GPS position mode, detail info please refer to QL_LOC_POS_MODE_INFO_T */
int QL_LOC_Set_Position_Mode(uint32_t h_loc, QL_LOC_POS_MODE_INFO_T *pt_mode);

/* Start navigation, same as AT+QGPS=1, NMEA port start outputing nmea data */
int QL_LOC_Start_Navigation(uint32_t h_loc);

/* Stop navigation, same as AT+QGPSEND,  NMEA port stop outputing nmea data */
int QL_LOC_Stop_Navigation(uint32_t h_loc);
...
int QL_LOC_Get_Current_Location(uint32_t h_loc, QL_LOC_LOCATION_INFO_T *pt_loc_info, int timeout_sec);

/* Init LOC module and return the h_loc, this should be called before any other QL_LOC_xxx api. */
int QL_LOC_Client_Init(uint32_t  *ph_loc);

/* DeInit LOC module and release resource, this should be called at last. */
int QL_LOC_Client_Deinit(uint32_t h_loc);

/* Deletes location-aiding data. */
int QL_LOC_Delete_Aiding_Data(  uint32_t              h_loc, 
                                E_QL_LOC_DELETE_AIDING_DATA_TYPE_T  flags);

/* Injects time. */
int QL_LOC_InjectTime(  uint32_t      h_loc,
                        QL_LOC_INJECT_TIME_INTO_T   *pt_info);

/* Injects Location. */
int QL_LOC_InjectLocation( uint32_t           h_loc,
