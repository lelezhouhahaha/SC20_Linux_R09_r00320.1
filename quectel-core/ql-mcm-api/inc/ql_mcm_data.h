...
typedef struct {
    ...
    ql_apn_pdp_type_e pdp_type;             /* !< Packet Data Protocol (PDP)
                                               type specifies the type of data payload
    ...
    char apn_name[QL_MCM_DATA_MAX_APN_LEN];        /* !< A string parameter that is a logical name used to
                                               select the GGSN
                                               and external packet data network. */
    char username[QL_MCM_DATA_MAX_USERNAME_LEN];    /* !< Username used during data network authentication. */
    char password[QL_MCM_DATA_MAX_PASSWORD_LEN];    /* !< Password to be used during data network authentication. */
} QL_MCM_DATA_APN_INFO_T;
...
E_QL_ERROR_CODE_T QL_MCM_DATA_GetAPN
(
    data_client_handle_type	h_data,
    uint8_t				profile_idx,
    char				*apn_name
...
//add default route by joe start
E_QL_ERROR_CODE_T QL_MCM_DATA_Add_Default_Route
(
    data_client_handle_type             h_data,
    QL_MCM_DATA_ROUTE_INFO_T            *pt_info
);
E_QL_ERROR_CODE_T QL_MCM_DATA_Add_Route_in_default_table
(
    data_client_handle_type             h_data,
    QL_MCM_DATA_ROUTE_INFO_T            *pt_info
);

//add default route by joe end
