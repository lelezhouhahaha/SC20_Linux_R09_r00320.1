...
)
{
    ...
    if (is_new_uuid) {
		...
		memset(vendor_uuid_info, 0, sizeof(struct st_vendor_info));
...
)
{
	...
	memset(gcs_params, 0, sizeof(*gcs_params));
                                 ...
                                 char *mixer_path_xml)
{
    if (strstr(snd_card_name, "msm8939-tapan")) {
        ...
        strlcpy(mixer_path_xml, MIXER_PATH_XML_SKUW,
                 ...
                 strstr(snd_card_name, "msm8909-tashalite")) {
        ...
        strlcpy(mixer_path_xml, MIXER_PATH_XML_BG,
            sizeof(MIXER_PATH_XML_BG));
        stdev->is_gcs = true;
    } else {
    ...
    */
    if ((strstr(snd_card_name, "msm8939") ||
        ...
        !strstr(snd_card_name, "msm8909-tasha") &&
...
static int init_be_dai_name_table(struct platform_data *my_data)
{
	...
	memset(my_data->be_dai_name_table, 0, size);
...
void *platform_stdev_init(sound_trigger_device_t *stdev)
{
	...
	memset(my_data, 0, sizeof(struct platform_data));
    my_data->stdev = stdev;
...
{
    struct platform_data *my_data = (struct platform_data *)platform;
    struct listnode *v_node, *temp_node, *gcs_node, *gcs_temp_node, *mk_node, *mk_temp_node;
    struct st_vendor_info* v_info;
    ...
    if (my_data) {
        list_for_each_safe(v_node, temp_node, &my_data->stdev->vendor_uuid_list) {
			...
			if ((&v_info->gcs_usecase_list)->next == NULL)
				goto next;
            /* remove and free the gcs uid params */
...
next:
        ...
        list_for_each_safe(mk_node, mk_temp_node, &my_data->acdb_meta_key_list) {
...
)
{
	...
	memset(calib.buff, 0, calib.buff_size);
...
)
{
    ...
    case ST_DEVICE_CAL:
        ...
        if (is_hwmad_backend(platform, st_device, exec_mode)) {
...
)
{
    ...
    if (!stdev->sw_mad) {
        if (!stdev->conc_capture && stdev->tx_concurrency_active > 0)
...
int platform_stdev_get_channel_count
(
    void *platform __unused,
    const struct st_vendor_info *v_info
)
{
    int channel_count = SOUND_TRIGGER_CHANNEL_MODE_MONO;
    /*
...
)
{
    ...
    *backend_cfg_change = false;
    ...
    channel_count = platform_stdev_get_channel_count(platform, v_info);
...
bool platform_stdev_check_concurrency_supported
(
   void *platform,
...
)
{
    ...
    if (!config)
        return conc_allowed;
    if (config->u.usecase.type == USECASE_TYPE_PCM_PLAYBACK) {
        conc_allowed = true;
    } else if (stdev->conc_capture) {
        conc_allowed = true;
    ...
    ALOGV("%s: capture concurrency %s allowed, usecase type %d", __func__,
          conc_allowed ? "": "not", config->u.usecase.type);
    return conc_allowed;
}
