...
*
*******************************************************************************/
#define LOG_TAG    "Mango-core/data.c"
...
int lwm2m_data_decode_string(const lwm2m_data_t * dataP,
								    char * string,
								    size_t length)
{
	int result = 0;
    ...
    {
    case LWM2M_TYPE_INTEGER:
		utils_intToText(dataP->value.asInteger, (uint8_t *)string, length);
        result = 1;
        break;

    case LWM2M_TYPE_STRING:
	case LWM2M_TYPE_OPAQUE:
		LOG_ARG("dataP->value.asBuffer.length=%d", dataP->value.asBuffer.length);
		utils_stringCopy(string, MIN(dataP->value.asBuffer.length,length),(char *)dataP->value.asBuffer.buffer);
		result = 1;
        break;
    default:
        return 0;
...
void lwm2m_data_encode_int(int64_t value,
                           lwm2m_data_t * dataP)
{
   // LOG_ARG("value: %" PRId64 "", value);//joe del
                          ...
                          int64_t * valueP)
{
    ...
    //LOG_ARG("result: %d, valueP: %" PRId64, result, *valueP);//joe del
