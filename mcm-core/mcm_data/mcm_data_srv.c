...
static qmi_csi_cb_error  mcm_data_get_apn_name
(
  qmi_mcm_data_client_info_type  *clnt_info,
  qmi_req_handle                 req_handle,
  int                            msg_id,
...
) =
{
  ...
  /* Request handler for message ID C */
  mcm_data_get_apn_name,
  /* Request handler for message ID D */
  NULL
};
...
)
{
	...
	mcm_data_get_apn_name_req_msg_v01 *apn_name_req;
	mcm_data_get_apn_name_rsp_msg_v01 apn_name_resp;

	mcm_data_call_table_entry *call_entry;

	MCM_DATA_LOG_FUNC_ENTRY();

	memset(&apn_name_resp, 0x0, sizeof(mcm_data_get_apn_name_rsp_msg_v01));
	apn_name_req = (mcm_data_get_apn_name_req_msg_v01 *)req_c_struct;

	call_entry = (mcm_data_call_table_entry *)malloc(sizeof(mcm_data_call_table_entry));
	if (call_entry == NULL) {
		MCM_DATA_LOG_ERROR("%s", "Failed to malloc call_entry");
		apn_name_resp.apn_name_valid = FALSE;
		apn_name_resp.resp.error = MCM_ERROR_NO_MEMORY_V01;
		apn_name_resp.resp.result = MCM_RESULT_FAILURE_V01;
		goto error2;
	}

	if (apn_name_req->profile_index < 0 || apn_name_req->profile_index > 255) {
		MCM_DATA_LOG_ERROR("%s", "profile index is invalid");
		apn_name_resp.apn_name_valid = FALSE;
		apn_name_resp.resp.error = MCM_ERROR_INVALID_ARG_V01;
		apn_name_resp.resp.result = MCM_RESULT_FAILURE_V01;
		goto error1;
	}
	...
	if ((call_entry->dsi_handle =
				dsi_get_data_srvc_hndl(&mcm_data_dsi_net_ev_cb, (void*)call_entry)) == NULL)
	{
		/* dsi_handle was not generated. */
		MCM_DATA_LOG_ERROR("%s", "Failed to get dsi_handle");
		apn_name_resp.apn_name_valid = FALSE;
		apn_name_resp.resp.error = MCM_ERROR_MCM_SERVICES_NOT_AVAILABLE_V01;
		apn_name_resp.resp.result = MCM_RESULT_FAILURE_V01;
		goto error1;
	}

	//MCM_DATA_LOG_INFO("Obtained dsi_hndl : %p", call_entry->dsi_handle);

	/* Save client handle into this empty entry. */
	call_entry->qmi_cli_handle = clnt_info->clnt;
	call_entry->self = call_entry;

	rc = dsi_data_get_apn_name(call_entry->dsi_handle, apn_name_req->profile_index, apn_name_resp.apn_name);
	if (rc == DSI_SUCCESS)
	{
		//MCM_DATA_LOG_INFO ("Peeta r1 got apn name %s", apnName);
		apn_name_resp.apn_name_valid = TRUE;
		apn_name_resp.apn_name_len = MIN(strlen(apn_name_resp.apn_name), MCM_DATA_MAX_APN_LEN_V01);

		apn_name_resp.resp.result = MCM_RESULT_SUCCESS_V01;
		apn_name_resp.resp.error = MCM_SUCCESS_V01;
	} else {
		apn_name_resp.apn_name_valid = FALSE;
		apn_name_resp.resp.error = MCM_ERROR_UNKNOWN_V01;
		apn_name_resp.resp.result = MCM_RESULT_FAILURE_V01;
	}


	MCM_DATA_LOG_INFO ("Server got apn name %s with error code %d, "
			"result %d, apn_name_resp.apn_name_valid %d",
			apn_name_resp.apn_name,
			apn_name_resp.resp.error,
			apn_name_resp.resp.result,
			apn_name_resp.apn_name_valid);

error1:

	qmi_csi_send_resp(req_handle, msg_id, &apn_name_resp,
			sizeof(mcm_data_get_apn_name_rsp_msg_v01));
