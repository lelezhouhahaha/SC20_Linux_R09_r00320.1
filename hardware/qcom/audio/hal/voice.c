...
int voice_start_usecase(struct audio_device *adev, audio_usecase_t usecase_id)
{
	...
	ALOGD("[%s:%d] usecase_id = %d device = %d", __func__, __LINE__, usecase_id, uc_info->devices);
	...
	//peeta
	ALOGD("[%s:%d] pcm_dev_rx_id = %d", __func__, __LINE__, pcm_dev_rx_id);
	ALOGD("[%s:%d] pcm_dev_tx_id = %d", __func__, __LINE__, pcm_dev_tx_id);
    ...
    ALOGD("voice_config.rate %d, sample_rate = %d\n", voice_config.rate, sample_rate);

	//ALOGD("[%s:%d] voice.mic_mute = %d", __func__, __LINE__, adev->voice.mic_mute);
...
bool voice_is_call_state_active(struct audio_device *adev)
{
	...
	ALOGD("[%s:%d] call_state = %d", __func__, __LINE__, call_state);
