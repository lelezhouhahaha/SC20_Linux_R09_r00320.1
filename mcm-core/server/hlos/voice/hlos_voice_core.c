...
void hlos_voice_ind_hdlr_all_call_status_ind()
{
    ...
    do
    {
       ...
       for (i=0; i<call_list_ptr->num_of_calls; i++)
       {
           ...
           //add by Raye.shi start
           if(CRI_VOICE_CALL_STATE_ACTIVE == call_obj_ptr->cri_call_state)//0
           {
             if(1 == audio_enabled)
             {
                UTIL_LOG_MSG("\n\n call state active DIAL_REQ DISABLE_AUDIO_STREAM\n");
                //mcm_srv_disable_audio_stream();//peeta
                audio_enabled = 0;
             }
             if (0 == voice_enabled)
             {
           ...
           if(CRI_VOICE_CALL_STATE_DIALING == call_obj_ptr->cri_call_state )//2
           {
              /*if (1 == audio_enabled)
              {
           ...
           if(CRI_VOICE_CALL_STATE_END == call_obj_ptr->cri_call_state )//7
          {
              if (1 == audio_enabled)
              {
...
void hlos_voice_auto_answer_needed()
{
   ...
   if ( 1 == voice_enabled)
   {
       ...
       //mcm_srv_disable_voice_stream();//peeta
       voice_enabled = 0;
   }
   if ( 0 == audio_enabled)
   {
       UTIL_LOG_MSG("\n\n INCOMING CALL,ENABLE_AUDIO_STREAM\n\n");
       //mcm_srv_enable_audio_stream();//peeta
       audio_enabled = 1;
   }
}
