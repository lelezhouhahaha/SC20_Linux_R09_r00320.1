...
void hlos_nas_send_registration_update_to_client()
{
    ...
    //mcm_nw_signal_strength_event_ind_msg_v01 signal_ind_msg;
    ...
    //memset(&signal_ind_msg, NIL,sizeof(signal_ind_msg));
...
void hlos_nas_get_operator_name_request_handler(void *event_data)
{
    ...
    hlos_core_hlos_request_data = (hlos_core_hlos_request_data_type*)event_data;
    memset(&hlos_resp, 0,sizeof(hlos_resp));
...
void hlos_nas_set_sys_sel_pref_handler(void *event_data)
{
    int i;
    qmi_error_type_v01 ret_val = QMI_ERR_INTERNAL_V01;

    mcm_nw_set_sys_sel_pref_req_msg_v01 *hlos_req_msg;
    mcm_nw_set_sys_sel_pref_resp_msg_v01 hlos_resp_msg;
    hlos_core_hlos_request_data_type *hlos_core_hlos_request_data;
    hlos_core_hlos_request_data = (hlos_core_hlos_request_data_type*) event_data;
    hlos_req_msg = (void *)hlos_core_hlos_request_data->data;
    nas_set_system_selection_preference_req_msg_v01 nas_req_msg;
    nas_set_system_selection_preference_resp_msg_v01 nas_resp_msg;
    ...
    if (hlos_req_msg->emergency_mode_valid == TRUE) {
        nas_req_msg.emergency_mode_valid = TRUE;
        nas_req_msg.emergency_mode = hlos_req_msg->emergency_mode;
    }

    if (hlos_req_msg->change_duration_valid == TRUE) {
        nas_req_msg.change_duration_valid = TRUE;
        if (hlos_req_msg->change_duration == 0)
            nas_req_msg.change_duration = NAS_POWER_CYCLE_V01;
        else
            nas_req_msg.change_duration = hlos_req_msg->change_duration;
    }

    if (hlos_req_msg->srv_domain_pref_valid == TRUE) {
        nas_req_msg.srv_domain_pref_valid = TRUE;
        nas_req_msg.srv_domain_pref = hlos_req_msg->srv_domain_pref;
    }
    ...
    UTIL_LOG("%s:%d - nas_resp_msg.resp.result = %d\n", nas_resp_msg.resp.result);
...
void hlos_nas_set_dual_standby_pref_response_handler(cri_core_context_type context,
        cri_core_error_type cri_core_error,
        void *hlos_cb_data,
        void *cri_resp_data)
{
    mcm_nw_set_dual_standby_pref_resp_msg_v01 hlos_resp_msg;

    UTIL_LOG_MSG("hlos_nas_set_dual_standby_pref_response_handler enter\n");
    ...
    if (cri_core_error == CRI_ERR_NONE_V01) {
        ...
        hlos_resp_msg.response.result = MCM_RESULT_FAILURE_V01;
    }

    hlos_resp_msg.response.error = hlos_map_qmi_ril_error_to_mcm_error(cri_core_error);
    hlos_core_send_response(NIL,
                            cri_core_error,
                            hlos_cb_data,
                            &hlos_resp_msg,
...
void hlos_nas_set_dual_standby_pref_async_handler(void *event_data)
{
    int i;
    qmi_error_type_v01 ret_val = QMI_ERR_INTERNAL_V01;
    cri_core_context_type cri_core_context = NIL;

    mcm_nw_set_dual_standby_pref_req_msg_v01 *hlos_req_msg;
    mcm_nw_set_dual_standby_pref_resp_msg_v01 *hlos_resp_msg = NULL;
    ...
    hlos_core_hlos_request_data = (hlos_core_hlos_request_data_type*) event_data;
    hlos_req_msg = (void *)hlos_core_hlos_request_data->data;
    hlos_resp_msg = (mcm_nw_set_dual_standby_pref_resp_msg_v01 *)util_memory_alloc(sizeof(*hlos_resp_msg));

    ret_val = cri_nas_set_dual_standby_handler(cri_core_context, hlos_req_msg->card, event_data, hlos_nas_set_dual_standby_pref_response_handler);
    if (QMI_ERR_NONE_V01 != ret_val) {
        if (QMI_ERR_NONE_V01 != ret_val) {
            if (hlos_resp_msg)
            {
                hlos_resp_msg->response.result = MCM_RESULT_FAILURE_V01;
                hlos_resp_msg->response.error = hlos_map_qmi_ril_error_to_mcm_error(ret_val);
                // in case of error send response immediately. if success, async resp cb will send response to client.
                UTIL_LOG_MSG("hlos_nas_network_scan_request_handler Error - %d\n", ret_val);
                hlos_core_send_response(NIL,
                                        NIL,
...
void hlos_nas_set_dual_standby_pref_handler(void *event_data)
{
    int card;
    qmi_error_type_v01 ret_val = QMI_ERR_INTERNAL_V01;
    cri_core_context_type cri_core_context = NIL;
    ...
    if (event_data == NULL) {
        UTIL_LOG_MSG("hlos_nas_set_dual_standby_pref_handler event_data is NULL\n");
        return;
    }
    ...
    if(QMI_ERR_NONE_V01 != ret_val) {
        UTIL_LOG("hlos_nas_set_dual_standby_pref_handlerr error - %d\n", ret_val);
    }
