 ...
 *
 *******************************************************************************/
#define LOG_TAG "Mango-tinydtls/dtls.c"
...
calculate_key_block(dtls_context_t *ctx, 
  ...
  case TLS_PSK_WITH_AES_128_CCM_8: {
    ...
    int len;
	LOGD("calculate_key_block handshake->keyx.psk.identity = %s, handshake->keyx.psk.id_length = %d", handshake->keyx.psk.identity, handshake->keyx.psk.id_length);
...
dtls_handle_message(dtls_context_t *ctx, 
  ...
  while ((rlen = is_record(msg,msglen))) {
    ...
    LOGD("receive header", msg, sizeof(dtls_record_header_t));
    LOGD("receive unencrypted", data, data_length);
    ...
    case DTLS_CT_APPLICATION_DATA:
      ...
      LOGD("DTLS_CT_APPLICATION_DATA X");
      if (!peer) {
      ...
      LOGD("DTLS_CT_APPLICATION_DATA E");
      break;
    default:
