...
//add by joe start
                ...
                if(MBEDTLS_SSL_MSG_ALERT == ssl->in_msgtype && 
                    ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM &&
                    MBEDTLS_MODE_CBC == mbedtls_cipher_get_cipher_mode(&ssl->transform_in->cipher_ctx_dec) &&
                    ssl->in_msglen == 2){
                        MBEDTLS_SSL_DEBUG_MSG( 1, ( "alert msg" ) );
                        return 0;// MBEDTLS_ERR_SSL_FATAL_ALERT_MESSAGE
                    }
#endif
//add by joe end
...
static int ssl_get_next_record( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM )
    {
        ...
        ssl->in_left = 0;
    //add by joe start
#if defined(VZW_FATAL_ALERT_SUPPORT) && defined(MBEDTLS_SSL_PROTO_DTLS)
    if(MBEDTLS_SSL_MSG_ALERT == ssl->in_msgtype && 
        ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM &&
        ssl->in_msglen == 2){
            mbedtls_cipher_mode_t mode = mbedtls_cipher_get_cipher_mode(&ssl->transform_in->cipher_ctx_dec);
            int message_size = mbedtls_ssl_hdr_len(ssl) + ssl->in_msglen;
            MBEDTLS_SSL_DEBUG_MSG( 2, ( "MSG_ALERT cyber mode %d", mode ) );
            MBEDTLS_SSL_DEBUG_BUF( 4, "input record from networks ", 
                                      ssl->in_hdr,mbedtls_ssl_hdr_len(ssl) + ssl->in_msglen );
            if(mode == MBEDTLS_MODE_CBC){
                ssl->in_msg[0] = ssl->in_hdr[message_size-2];
                ssl->in_msg[1] = ssl->in_hdr[message_size-1];
                return(0);
            }
            MBEDTLS_SSL_DEBUG_MSG(2, ("in_msg: {%d:%d}",
