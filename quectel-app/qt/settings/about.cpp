...
int About::getbpVersion4Thread()
{
    ...
    printf("QLMISC_SendAT \"%s\" ret=%d with resp=\n%s\n", atc_cmd_req, ret, atc_cmd_resp);
    if (ret < 0) {
...
int About::getIMEIandMEID4Thread()
{
	...
	memset(atc_cmd_req,  0, sizeof(atc_cmd_req));
	memset(atc_cmd_resp, 0, sizeof(atc_cmd_resp));
	strcpy(atc_cmd_req,"AT+EGMR=0,7");
	ret = QLMISC_SendAT(atc_cmd_req, atc_cmd_resp, ATC_RESP_CMD_MAX_LEN);
	printf("QLMISC_SendAT \"%s\" ret=%d with resp=\n%s\n", atc_cmd_req, ret, atc_cmd_resp);
	...
	memset(atc_cmd_req,  0, sizeof(atc_cmd_req));
	memset(atc_cmd_resp, 0, sizeof(atc_cmd_resp));
	strcpy(atc_cmd_req,"AT^CGSN");
	ret = QLMISC_SendAT(atc_cmd_req, atc_cmd_resp, ATC_RESP_CMD_MAX_LEN);
	printf("QLMISC_SendAT \"%s\" ret=%d with resp=\n%s\n", atc_cmd_req, ret, atc_cmd_resp);

    qStr = QString(atc_cmd_resp);
    list = qStr.split("\r\n", QString::QString::SkipEmptyParts);
    for (i = 0; i < list.size();i++) {
        qDebug() << list.at(i);
        if (list.at(i).contains("0x")) {
			rx.setPattern("0x\\S+");
			pos = rx.indexIn(qStr, 0);
            newStr = "MEID: ";
            newStr.append(qStr.mid(pos + 2, 16));
            setMEID(newStr);
