...
static int test_data(void)
{
    ...
    while(1)
    {
        ...
        case 22:
            ...
            break;
        case 23://QL_MCM_DATA_GetAPN
		{
            char apn[QL_MCM_DATA_MAX_APN_LEN]; //len <150
			int profile_idx = 1;

            printf("please input profile index: ");
            scanf("%d", &profile_idx);

			memset(apn, 0, sizeof(apn));

            ret = QL_MCM_DATA_GetAPN(h_data, profile_idx, apn);
			if (ret == 0)
				printf("QL_MCM_DATA_GetAPN ret apn name is: (%s)\n", apn);
			else
				printf("QL_MCM_DATA_GetAPN ret = %d\n", ret);
            break;
		}
        default:
            show_group_help(&t_data_test);
