...
#define WAV_RIFF_CHUNK_ID       0x52494646
#define WAV_FORMAT_CHUNK_ID     0x666d7420
#define WAV_FACT_CHUNK_ID       0x66616374
#define WAV_DATA_CHUNK_ID       0x64617461
#define WAV_WAVE_ID             0x57415645
#endif

#ifdef WAV_ENDIAN_BIG
#define WAV_RIFF_CHUNK_ID       0x46464952
#define WAV_FORMAT_CHUNK_ID     0x20746d66
#define WAV_FACT_CHUNK_ID       0x74636166
#define WAV_DATA_CHUNK_ID       0x61746164
...
WavFile* wav_open(WAV_CONST char* filename, WavU32 mode)
{
	if (mode == WAV_OPEN_READ) {
		if (filename == NULL || strlen(filename) == 0) {
			fprintf(stderr, "[Error] filename is invalid\n");
			return NULL;
		}

		if (access(filename, F_OK)) {
			fprintf(stderr, "[Error] filename is not exist\n");
			return NULL;
		}
	}
...
size_t wav_read_LR_channels(WavFile* self, WavFile* self2, void *buffer, size_t count)
{
    size_t tmp_len_remain;
    size_t read_count;
    int ret = 0;
    int i = 0;
    ...
    WavU16 n_channels2 = wav_get_num_channels(self2);
    size_t sample_size2 = wav_get_sample_size(self2);
    size_t len_remain2;
    void *buff2 = NULL;


    if(n_channels != 2 || sample_size != 2 || n_channels2 != 2 || sample_size2 != 2)
        return 0;

    if (!(self->mode & WAV_OPEN_READ) || !(self2->mode & WAV_OPEN_READ)) {
        wav_err_set_literal(WAV_ERR_MODE, "This WavFile is not readable");
        return 0;
    }

    if ((self->format_chunk.body.format_tag == WAV_FORMAT_EXTENSIBLE) || (self2->format_chunk.body.format_tag == WAV_FORMAT_EXTENSIBLE)) {
        wav_err_set_literal(WAV_ERR_FORMAT, "Extensible format is not supported");
        return 0;
    }
    ...
    len_remain2 = wav_get_length(self2) - (size_t)wav_tell(self2);
    if (g_err.code != WAV_OK)
        return 0;

    tmp_len_remain = ((len_remain < len_remain2) && (len_remain != 0)) ? len_remain : (len_remain2 != 0) ? len_remain2 : len_remain;

    count = (count <= tmp_len_remain) ? count : tmp_len_remain;
    if (count == 0)
        return 0;

    buff = (void *)malloc(sample_size * n_channels * count);
    if(buff == NULL)
        return 0;

    buff2 = (void *)malloc(sample_size2 * n_channels2 * count);
    if(buff2 == NULL) {
        free(buff);
        return 0;
    }

    memset(buff, 0, sample_size * n_channels * count);
    if(len_remain > 0) {
        read_count = fread(buff, sample_size, n_channels * count, self->fp);
        if (ferror(self->fp)) {
            wav_err_set(WAV_ERR_OS, "Error when reading %s [errno %d: %s]", self->filename, errno, strerror(errno));
            free(buff);
            free(buff2);
    ...
    memset(buff2, 0, sample_size2 * n_channels2 * count);
    if(len_remain2 > 0) {
        read_count = fread(buff2, sample_size2, n_channels2 * count, self2->fp);
        if (ferror(self2->fp)) {
            wav_err_set(WAV_ERR_OS, "Error when reading %s [errno %d: %s]", self->filename, errno, strerror(errno));
            free(buff);
            free(buff2);
    ...
    //copy buff2 right 2bytes data to buff right 2bytes data
    for(i = 0; i < read_count/2; i++) {
        memcpy((char *)buff+4*i, (char *)buff2+4*i, sample_size2);
    }

    memcpy((char *)buffer, (char *)buff, sample_size * n_channels * count);
