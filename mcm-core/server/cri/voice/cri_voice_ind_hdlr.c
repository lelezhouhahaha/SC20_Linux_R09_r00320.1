...
)
{
    ...
    if( NULL != call_status_ind_ptr && NULL != call_list )
    {
        ...
        for ( i = 0; i < call_status_ind_ptr->call_info_len; i++ )
        {
            ...
            switch ( iter_qmi_call_info->call_state )
            {
                ...
                case CALL_STATE_CONVERSATION_V02:
                    ...
                    //add by peeta
                    if (CALL_TYPE_EMERGENCY_IP_V02 == iter_qmi_call_info->call_type) {
                        QCRIL_LOG_INFO("Peeta %s:%d", __func__, __LINE__);
                ...
                case CALL_STATE_END_V02:
                    ...
                    if( (CALL_TYPE_EMERGENCY_V02 ==iter_qmi_call_info->call_type) ||
                        (CALL_TYPE_EMERGENCY_IP_V02 == iter_qmi_call_info->call_type) ||
        ...
        if(hlos_ind_cb_func_ptr)
        {
            ...
            if (is_e911_call_ended && iter_qmi_call_info->call_type == CALL_TYPE_EMERGENCY_IP_V02) {
                QCRIL_LOG_INFO("Peeta is_e911_call_ended %d", is_e911_call_ended);
                e911_state_init = CRI_VOICE_E911_INACTIVE_V01;
                hlos_ind_cb_func_ptr(
                    CRI_VOICE_E911_STATE_IND,
                    (void*)&e911_state_init,
                    sizeof(e911_state_init));
...
)
{
    ...
    if (NULL != dtmf_ind_ptr) {
        ...
        cri_dtmf_ind.dtmf_info.dtmf_event = dtmf_ind_ptr->dtmf_info.dtmf_event;
