...
/*===========================================================================
  FUNCTION:  dsi_iface_ioctl_get_modem_iface_mtu
===========================================================================*/
/*!
    @brief
    Get the IFACE MTU from MODEM for the specified IP family.

    @parameters
    qdi_hndl   - qdi handle of interface
    ip_version - IP version for MTU is queried
    argval_ptr - Pointer to MTU variable (unsigned long)
...
*/
/*=========================================================================*/
static int dsi_iface_ioctl_get_modem_iface_mtu
(
  qdi_client_handle_t qdi_hndl,
  int ip_version,
  void *argval_ptr
)
{
  uint16_t                               req_mask = 0;
  wds_get_runtime_settings_resp_msg_v01  call_info;
  uint32_t                              *iface_mtu = NULL;
  int                                    ret = DSI_ERROR;

  DSI_LOG_DEBUG("dsi_iface_ioctl_get_modem_iface_mtu(): ENTRY QDI handle [%d]", qdi_hndl);

  if( NULL == argval_ptr || NULL == qdi_hndl)
  {
    DSI_LOG_ERROR("%s","dsi_iface_ioctl_get_modem_iface_mtu(): Invalid parameters passed [NULL]");
    DSI_LOG_DEBUG("%s","dsi_iface_ioctl_get_modem_iface_mtu(): EXIT with error" );
    return DSI_ERROR;
  }

  iface_mtu = (uint32_t*) argval_ptr;
  *iface_mtu = 0;

  memset(&call_info, 0, sizeof(wds_get_runtime_settings_resp_msg_v01));

  /* Set appropriate mask to get the MTU from MODEM */
  req_mask = QMI_WDS_MASK_REQ_SETTINGS_MTU_V01;

  /* Get MTU from MODEM */
  if(DSI_SUCCESS != dsi_get_curr_call_info(qdi_hndl, req_mask, ip_version, &call_info))
  {
    DSI_LOG_ERROR("%s","dsi_iface_ioctl_get_modem_iface_mtu(): EXIT error");
    return DSI_ERROR;
  }
  ...
  {
    *iface_mtu = call_info.mtu;
  }

  DSI_LOG_DEBUG("%s","dsi_iface_ioctl_get_modem_iface_mtu(): EXIT success");
  return DSI_SUCCESS;
}
  ...
  switch (ioctl_name)
  {
    ...
    case DSI_IFACE_IOCTL_REMOVE_DELEGATED_IPV6_PREFIX:
      ret = dsi_iface_ioctl_remove_delegated_ipv6_prefix(DSI_GET_QDI_HNDL(st->priv.dsi_iface_id),
                                                         argval_ptr);
      break;
    case DSI_IFACE_IOCTL_GET_MODEM_IFACE_V4_MTU:

      if (DSI_IP_VERSION_4 == st->priv.ip_version ||
          DSI_IP_VERSION_4_6 == st->priv.ip_version)
      {
        ret = dsi_iface_ioctl_get_modem_iface_mtu(DSI_GET_QDI_HNDL(st->priv.dsi_iface_id),
                                                  AF_INET,
                                                  argval_ptr);
      }
      break;
    case DSI_IFACE_IOCTL_GET_MODEM_IFACE_V6_MTU:
...
)
{
  ...
  DSI_GLOBAL_LOCK;
  DSI_LOG_DEBUG( "%s", "rel_data_srvc_hndl: LOCK" ); //add by peeta
...
int dsi_data_get_apn_name(dsi_hndl_t hndl, unsigned char profile_idx, char *apn_name)
{
	int ret = DSI_ERROR;
	dsi_store_t *st_hndl = NULL;

	DSI_L2S_ENTRY_PARAMS("dsi_hndl=%p", (unsigned int*)hndl);
	DSI_LOG_DEBUG( "%s", "dsi_data_get_apn_name: ENTRY" );

	st_hndl = ( (dsi_store_t *) hndl );

	if (dsi_profile_get_apn_name(st_hndl, profile_idx, apn_name) == 0)
		ret = DSI_SUCCESS;
