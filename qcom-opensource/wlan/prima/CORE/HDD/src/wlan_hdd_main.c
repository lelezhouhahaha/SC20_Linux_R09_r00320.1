...
//2020.3.6 added by tommy
#define GET_MAC_FROM_PARAM      1
static char *nv_mac="00:00:00:00:00:00";
...
//2020.3.6 added by tommy
#ifdef GET_MAC_FROM_PARAM
    printk("WIFI MAC:%s\n",nv_mac);
    sscanf(nv_mac,"%x:%x:%x:%x:%x:%x",&mac_addr.bytes[0],&mac_addr.bytes[1],&mac_addr.bytes[2],
    &mac_addr.bytes[3],&mac_addr.bytes[4],&mac_addr.bytes[5]);
    if(!vos_is_macaddr_zero(&mac_addr))
    {
        pHddCtx->cfg_ini->intfMacAddr[0] = mac_addr;
    }else{
        printk("WIFI MAC is invalid.\n");
#endif
...
//2020.3.6 added by tommy
module_param(nv_mac, charp, S_IRUGO);
