...
st_api_test_case at_gps_testlist[] = 
{
    ...
    {19,   "QL_LOC_Example_ClearAllData"}, 
    {20,   "QL_LOC_Example_GetPos_WithXtra"},
    {21,   "QL_LOC_Example_GetPos"},   
...
QL_LOC_LOCATION_INFO_T  t_loc_info  = {0};
    ...
    while(1)
    {
	    ...
	    //long long utime=0;
	    printf("please input utc time(ms):\n");
	    scanf("%lld",&utime);
            //You should fill the right time of xtra data
            t_info.time = utime;
            t_info.time_reference = 0;
            t_info.uncertainty = 3500;
            
            ret = QL_LOC_InjectTime(h_loc, &t_info);
            printf("QL_LOC_InjectTime:%lld(ms) ret=%d\n",utime, ret);
            break;
        }
        case 10://"QL_LOC_InjectLocation"
        {
            ...
            t_info.longitude = 117.197854;//You should fill this with right value
            t_info.latitude  = 38.111023;
            t_info.accuracy  = 50.0;
	...
	case 19:
		...
		printf("QL_LOC_AddRxIndMsgHandler ret=%d\n", QL_LOC_AddRxIndMsgHandler(ql_loc_rx_ind_msg_cb, (void*)h_loc));
		printf("QL_LOC_Set_Indications ret %d\n", ret = QL_LOC_Set_Indications(h_loc, 0x1FF));

		printf("QL_LOC_Delete_Aiding_Data ret=%d\n",QL_LOC_Delete_Aiding_Data(h_loc, 0xFFFFFFFF));
	...
	case 20:
	{
       ...
       //You need set it as you wish
		...
		printf("QL_LOC_Set_Position_Mode ret %d\n", ret = QL_LOC_Set_Position_Mode(h_loc, &t_mode));
		 printf("QL_LOC_Delete_Aiding_Data ret=%d\n",QL_LOC_Delete_Aiding_Data(h_loc, 0xFFFFFFFF));
		 system("rm /data/xtra3grc.bin;wget http://xtrapath3.izatcloud.net/xtra3grc.bin -P /data/");
		time(&timep);
		utime=timep;
		t_info.time = utime*1000;
            t_info.time_reference = 0;
            t_info.uncertainty = 3500;
		//printf("QL_LOC_InjectTime:%lld(ms) ret=%d\n",t_info.time,QL_LOC_InjectTime(h_loc, &t_info));
		
		printf("QL_LOC_Xtra_InjectFile ret=%d\n", QL_LOC_Xtra_InjectFile(h_loc, "/data/xtra3grc.bin"));
		ret = QL_LOC_Get_Current_Location(h_loc, &t_loc_info, timeout_sec);
            printf(" QL_LOC_Get_Current_Location ret %d\n", ret);
            if(ret < 0)
            {
		...
		time(&timep);
		utime=timep-utime;
                printf("**** Latitude = %lf, Longitude=%lf, altitude=%lf, accuracy = %f time:%d s****\n", 
                        t_loc_info.latitude, t_loc_info.longitude, t_loc_info.altitude, t_loc_info.accuracy,utime);
            }
	     printf("QL_LOC_Client_Deinit:%d\n",QL_LOC_Client_Deinit(h_loc));
	    break;
	}
