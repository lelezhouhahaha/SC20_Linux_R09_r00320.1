...
static uint8_t prv_location_discover
(
uint16_t instanceId,
int * numDataP,
...
lwm2m_object_t * get_object_location(void)
{
    ...
    if (NULL != locationObj)
    {
        ...
        if (NULL != locationObj->userData)
        {
            ...
            //init some default location data
            data->speed = 10.0;
            data->latitude = -20.0;
            data->longitude = 201.0;
            data->altitude = -108.0;
            data->radius = 15.0;
            data->bearing = 100;
            QL_LWM2M_GetGps(data);

            hor_speed = ((data->speed)*18)/5;
            location_setVelocity(locationObj, data->bearing, hor_speed, 255); // 255: speedUncertainty not supported!
