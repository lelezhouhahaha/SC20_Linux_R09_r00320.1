...
void *qahw_load_module(const char *hw_module_id) {
    ALOGD("%s %d hal string(%s)", __func__, __LINE__, hw_module_id);
    Parcel data, reply;
    data.writeInterfaceToken(Iqti_audio_server::getInterfaceDescriptor());
...
int qahw_unload_module(qahw_module_handle_t *hw_module) {
    ALOGD("%s %d hal handle(%p)", __func__, __LINE__, hw_module);
    Parcel data, reply;
    data.writeInterfaceToken(Iqti_audio_server::getInterfaceDescriptor());
...
ssize_t qahw_out_write(qahw_stream_handle_t *stream,
    ...
    //ALOGV("Waking up stream thread(%p:%p)", p_stream, p_stream->n_stream_handle);
    rc = pthread_cond_signal(&shmem_data->c_cond);
    //ALOGV("Woken up stream thread(%p:%p) %d", p_stream, p_stream->n_stream_handle, (int)rc);

    while (shmem_data->status == -1) {
        //ALOGV("Waiting for status signal stream(%p:%p)", p_stream, p_stream->n_stream_handle);

        rc = pthread_cond_wait(&shmem_data->c_cond, &shmem_data->c_mutex);

        //ALOGV("Waiting ended for status signal stream(%p:%p) %d", p_stream, p_stream->n_stream_handle, (int)rc);
    }
...
int qahw_out_get_presentation_position(const qahw_stream_handle_t *out_handle,
    ...
    //wakeup server stream thread.
    //ALOGV("Waking up stream thread(%zd:%zd)", p_stream, p_stream->n_stream_handle);
    rc = pthread_cond_signal(&shmem_data->c_cond);
    //ALOGV("Woken up stream thread(%zd:%zd) %d", p_stream, p_stream->n_stream_handle, rc);

    while (shmem_data->status == -1) {
        //ALOGV("Waiting for status signal stream(%zd:%zd)", p_stream, p_stream->n_stream_handle);

        rc = pthread_cond_wait(&shmem_data->c_cond, &shmem_data->c_mutex);

        //ALOGV("Waiting ended for status signal stream(%zd:%zd) %d", p_stream, p_stream->n_stream_handle, rc);
    }
    rc = pthread_mutex_unlock(&shmem_data->c_mutex);
...
int qahw_create_audio_loopback(qahw_module_handle_t *hw_module,
                        uint32_t input,
    ...
    data.writeInterfaceToken(Iqti_audio_server::getInterfaceDescriptor());
    data.write(&hw_module, sizeof(qahw_module_handle_t *));
    data.writeUint32(input);

    uint32_t size = sizeof(struct audio_config);
    data.writeUint32(size);
    android::Parcel::WritableBlob blob;
    data.writeBlob(size, false, &blob);
    memset(blob.data(), 0x0, size);
    ...
    data.writeUint32(output);

    data.writeUint32(size);
    data.writeBlob(size, false, &blob);
    memset(blob.data(), 0x0, size);
    memcpy(blob.data(), sink, size);

    return remote()->transact(CREATE_AUD_LOOPBACK, data, &reply);
}

int qahw_release_audio_loopback(qahw_module_handle_t *hw_module) {
    Parcel data, reply;
    data.writeInterfaceToken(Iqti_audio_server::getInterfaceDescriptor());
    data.write(&hw_module, sizeof(qahw_module_handle_t *));
    return remote()->transact(RELEASE_AUD_LOOPBACK, data, &reply);
}
