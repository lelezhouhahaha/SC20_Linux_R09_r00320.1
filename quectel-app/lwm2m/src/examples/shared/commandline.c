...
#define LOG_TAG "Mango-shared/commandline.c"
                   ...
                   int indent)
{
    ...
    int i;
    char logcat_buffer[1024];
	memset(logcat_buffer, 0, sizeof(logcat_buffer));  // can not print if no memset
    ...
    while (i < length)
    {
		...
		int c;
		//for ( c = 0 ; c < indent ; c++)
		//	sprintf(logcat_buffer, "%s%s",logcat_buffer, "    ");

		//LOGD("logcat_buffer = %s", logcat_buffer);
        memcpy(array, buffer+i, 16);
        for (j = 0 ; j < 16 && i+j < length; j++)
        {
            fprintf(stream, "%02X ", array[j]);
            //LOGD("%02X ", array[j]);
			sprintf(logcat_buffer, "%s%02X ", logcat_buffer, array[j]);
            if (j%4 == 3) {
                fprintf(stream, " ");
                //LOGD(" ");
				sprintf(logcat_buffer, "%s%s",logcat_buffer, " ");
        ...
        for (j = 0 ; j < 16 && i+j < length; j++)
        {
            if (isprint(array[j])){
                ...
                //LOGD("%c", array[j]);
				sprintf(logcat_buffer, "%s%c",logcat_buffer, array[j]);
            } else {
                fprintf(stream, ".");
                //LOGD(".");
				sprintf(logcat_buffer, "%s%s",logcat_buffer, ".");
        ...
        fprintf(stream, "\n");
		//sprintf(logcat_buffer, "%s%s",logcat_buffer, "\n");
		LOGD("* %s", logcat_buffer);
		memset(logcat_buffer, 0, sizeof(logcat_buffer));
        i += 16;
    }
	//LOGD("logcat_buffer = %s", logcat_buffer);
}
