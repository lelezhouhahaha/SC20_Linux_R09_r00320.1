...
#if defined QUECTEL_QCFG_CMD
    { //AT command fwd type     // add for QCFG dempsey
                1, // Number of commands
                {
                        { QMI_ATCOP_AT_CMD_NOT_ABORTABLE, "+QCFG"},
                }
        },
#endif

#if defined QUECTEL_LWM2M_CMD
    { //AT command fwd type	// add for ODIS by Jambo,20200218
        1, // Number of commands
        {
            { QMI_ATCOP_AT_CMD_NOT_ABORTABLE, "+ODIS"},
        }
    },
#endif
};
...
void btproperty_restart(void)
{
	int pid=-1;
    pid=getProcessPidByName("btproperty");
	
	if(pid < 0)
	{
		system("systemctl start btproperty");
	}
...
void start_bluetooth()
{
	...
	int i;
       ...
       ret = stat("/data/misc/bluetooth/btprop", &stat_buf);
       if(ret==0)
       {
		btproperty_restart();
		usleep(500000);
		btproperty_restart();
               system("ftmdaemon -n -dd 2>/data/ftmd.log &");
       }
       else{
	    system("systemctl restart btproperty"); 
	     //system("killall btproperty");
             for(i=0;i<3;i++)
	     {
               //system("killall btproperty");
		usleep(400000);
		...
		if(ret==0)
			...
			usleep(500000);
			btproperty_restart();
		       system("ftmdaemon -n -dd 2>/data/ftmd2.log &");
                       printf("powell success!\n");
		       break;
               }
               else{
			//system("killall btproperty");
...
#ifdef QUECTEL_QCFG_CMD
		if (strcasecmp(cmd->name, "+QCFG") == 0) {
			quec_qcfg_handle(cmd, result);
		}
#endif

#ifdef QUECTEL_LWM2M_CMD
		if (strcasecmp(cmd->name, "+ODIS") == 0) {
			quec_odis_handle(cmd, result);
		}
#endif
